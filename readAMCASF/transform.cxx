...
void transition(double r[][4],float x,float y,float z){
	r[0][0]=1;       r[0][1]=0;       r[0][2]=0;       r[0][3]=x;
    r[1][0]=0;       r[1][1]=1;		  r[1][2]=0;       r[1][3]=y;
    r[2][0]=0;       r[2][1]=0     ;  r[2][2]=1;       r[2][3]=z;
    r[3][0]=0;       r[3][1]=0;       r[3][2]=0;       r[3][3]=1;
}
...
void loadIdentity(double r[][4]){
	for(int i=0;i<4;i++)
		for(int j=0;j<4;j++)
			r[i][j]=(i==j);
}
void matrixDuplicate(double r[][4],double d[][4]){
	for(int i=0;i<4;i++)
		for(int j=0;j<4;j++){
			d[i][j]=r[i][j];
		}
}
void matrixPremultiply(double d[][4],double s[][4]){// d = s*d
	double t[4][4];
	matrix_mult(s,d,t);
	matrixDuplicate(t,d);
}
void matrixPostmultiply(double d[][4],double s[][4]){//d = d*s
	double t[4][4];
	matrix_mult(d,s,t);
	matrixDuplicate(t,d);
}
